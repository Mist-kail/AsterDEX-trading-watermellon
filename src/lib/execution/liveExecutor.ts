...
export class LiveExecutor implements ExecutionAdapter {
  ...
  async closePosition(reason: string, meta?: Record<string, unknown>): Promise<void> {
    try {
      ...
      console.log(`[LiveExecutor] Attempting to close position: ${reason}`, {
        currentPositionAmt: positionAmt,
        side,
        quantity,
        ...meta
      });
      ...
      try {
        ...
        // Handle reduceOnly rejection - try without reduceOnly as fallback
        if (err.includes("ReduceOnly Order is rejected") || err.includes("-2022")) {
          console.log("[LiveExecutor] ReduceOnly rejected, trying regular order", {
            positionAmt,
            side,
            quantity,
            reason
          ...
          try {
            ...
            console.error("[LiveExecutor] Fallback order also failed", { fallbackErr });

            // If it's still about position side, try with positionSide
            if (fallbackErr.includes("position side")) {
        ...
        } else if (err.includes("position side")) {
          const positionSide = positionAmt > 0 ? "LONG" : "SHORT";
          await this.placeOrder({
            side,
            type: "MARKET",
      ...
      console.log(`[LiveExecutor] Successfully closed position: ${reason}`, { positionAmt, side, quantity, ...meta });
    } catch (error) {
  ...
  private async getCurrentPosition(): Promise<{ positionAmt: string; symbol: string; entryPrice?: string } | null> {
    try {
      ...
      if (position) {
        console.log(`[LiveExecutor] Position check: ${this.symbol} = ${position.positionAmt}`);
      ...
      console.log("[LiveExecutor] Primary position check failed, trying fallback", primaryError);
      try {
        const params = new URLSearchParams({
          symbol: this.symbol,
        });
        ...
        if (position) {
          console.log(`[LiveExecutor] Fallback position check: ${this.symbol} = ${position.positionAmt}`);
          if (position.positionAmt !== "0") {
          return position;
          }
        }
      } catch (fallbackError) {
        console.error("[LiveExecutor] Failed to get position from both endpoints", { primaryError, fallbackError });
      }
      return null;
    }
  }
