...
export class RestPoller {
  ...
  start(intervalMs: number = 5000): void { // Increased polling interval to reduce API load and timeout issues
    this.stop();
  ...
  private async fetchBalance(): Promise<void> {
    try {
      ...
      try {
        ...
        const now = Date.now();
        if (now - this.lastSuccessLog > 30000) {
          console.error(`[RestPoller] Balance polling failed repeatedly. This may affect risk management:`);
        console.error(`[RestPoller] Primary error: ${err.message}`);
        console.error(`[RestPoller] Fallback error: ${fallbackErr.message}`);
          console.error(`[RestPoller] Position polling still working - bot can continue trading`);
          this.lastSuccessLog = now;
        }
  ...
  private async signedRequest<T>(method: "GET" | "POST", endpoint: string, params: URLSearchParams): Promise<T> {
    ...
    // Increase recvWindow to 30 seconds to handle network latency
    params.append("recvWindow", "30000");
    ...
    try {
      const response = await fetch(url, {
        ...
        // Timeout handling is managed by the increased recvWindow
      });
