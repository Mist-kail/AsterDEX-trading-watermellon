...
let longSignals = 0;
bullishSequence.forEach((price, index) => {
  const bar: SyntheticBar = {
    startTime: Date.now() + index * 30000,
    endTime: Date.now() + (index + 1) * 30000,
    open: price,
    high: price + 0.5,
    low: price - 0.5,
    close: price,
  ...
  const signal = watermellonEngine.update(bar);
...
let shortSignals = 0;
bearishSequence.forEach((price, index) => {
  const bar: SyntheticBar = {
    startTime: Date.now() + index * 30000,
    endTime: Date.now() + (index + 1) * 30000,
    open: price,
  ...
  const signal = watermellonEngine2.update(bar);
